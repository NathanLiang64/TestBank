...
const TransferStaticDesignedAccount = () => {
  ...
  useEffect(async () => {
    const { add, edit } = clickMoreOptions;
    const { getDesignedAcct } = mockData;
    if (edit.click && edit.target) {
      const currenTarget = getDesignedAcct.designedAcctList.find((member) => member.id === edit.target);
...
export default TransferStaticDesignedAccount;
